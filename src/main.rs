...
fn submit_newgame(request: &mut Request) -> PencilResult {
    ...
    if !(home_score == 10 || away_score == 10) || home_score == away_score || home == away {
        ...
        context.insert("nykamp_fejl".to_string(), "Den indtastede kamp er ikke lovlig 😜".to_json());
        context.insert("heading".to_string(), "Fejl: Ny kamp".to_json());
        context.insert("body".to_string(), "Fejl ved registrering af ny kamp".to_json());
        return request.app.render_template("index.html", &context)
    }

    println!("{:?}", conn.execute("INSERT INTO games (home_id, away_id, home_score, away_score) VALUES (?, ?, ?, ?)",
...
fn games(request: &mut Request) -> PencilResult {
    let conn = Connection::open("ratings.db").unwrap();
    let mut stmt = conn.prepare("SELECT (SELECT name FROM players p WHERE p.id = g.home_id) AS home, (SELECT name FROM players p WHERE p.id = g.away_id) AS away, home_score, away_score FROM games g ORDER BY ID DESC").unwrap();
...
impl ToJson for Player {
    fn to_json(&self) -> Json {
        let mut m: BTreeMap<String, Json> = BTreeMap::new();
        m.insert("rating".to_string(), format!("{:.1}", self.rating.mu()).to_json());
        m.insert("sigma".to_string(), format!("{:.1}", self.rating.sigma()).to_json());
...
fn rating(request: &mut Request) -> PencilResult {
    ...
    let conn = Connection::open("ratings.db").unwrap();
    let mut stmt = conn.prepare("SELECT id, name from players").unwrap();
    let mut stmt2 = conn.prepare("SELECT home_id, away_id, home_score, away_score from games").unwrap();
    ...
    for g in stmt2.query_map(&[], |row| Game{
        home: row.get(0),
        away: row.get(1),
...
fn submit_newplayer(request: &mut Request) -> PencilResult {
    ...
    if name.is_empty() {
        ...
        context.insert("nyspiller_fejl".to_string(), "Den indtastede spiller er ikke lovlig 😜".to_json());
        context.insert("heading".to_string(), "Fejl: Ny spiller".to_json());
        context.insert("body".to_string(), "Fejl ved registrering af ny spiller".to_json());
        return request.app.render_template("index.html", &context)
    }
    println!("{:?}", conn.execute("INSERT INTO players (name) VALUES (?)",
        &[&*name]));
...
fn main() {
    ...
    app.route("/newplayer/submit", &[Post], "ny hestesandwcih", submit_newplayer);
